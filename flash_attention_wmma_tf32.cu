...
__global__
void flash_attention_kernel_wmma_tf32(
            ...
            for (int bi = 0; bi < col_tile_size; bi += WMMA_M) {
                for (int bj = 0; bj < row_tile_size; bj += WMMA_N) {
                    ...
                    for (int k = 0; k < d; k += WMMA_K) {
                        if (k + WMMA_K <= d) {
                            load_matrix_sync(a_frag, &Qi[bi * d + k], d);
                            load_matrix_sync(b_frag, &KjT[k * col_tile_size + bj], col_tile_size);
                ...
                for (int bj = 0; bj < d; bj += WMMA_N) {
                    ...
                    for (int k = 0; k < col_tile_size; k += WMMA_K) {
                        if (k + WMMA_K <= col_tile_size) {
                            load_matrix_sync(a_frag, &S[bi * col_tile_size + k], col_tile_size);
                            load_matrix_sync(b_frag, &Vj[k * d + bj], d);
                            mma_sync(c_frag, a_frag, b_frag, c_frag);
                        }
                    }
...
void launch_flash_attention_wmma_tf32(
    ...
    flash_attention_kernel_wmma_tf32<<<grid, block, shmem_size>>>(
